...
ALERT_COOLDOWN_SECS = 60 * 60  # 60 minutes
...
def maybe_send_trade_alerts(decisions):
    ...
    for decision in decisions:
        ...
        # Cooldown behavior:
        # - Mode3 OFF: block any repeat alert in cooldown window
        # - Mode3 ON : allow opposite action flips (e.g. BUY->SELL),
        #              still block same-direction duplicates
        if in_window:
            if not ALERT_MODE3:
                continue
...
def build_snapshot():
    ...
    for sym in symbols:
        ...
        # Defensive normalization: treat None/non-DataFrame as empty.
